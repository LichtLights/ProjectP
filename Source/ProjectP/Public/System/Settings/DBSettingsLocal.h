...
#include "DBSettingsLocal.generated.h"
...
USTRUCT()
struct FDBScalabilitySnapshot
{
	GENERATED_BODY()

	FDBScalabilitySnapshot();
 ...
 */
UCLASS()
class UDBSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EDBStatDisplayMode GetPerfStatDisplayState(EDBDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EDBDisplayablePerformanceStat Stat, EDBStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UDBSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EDBDisplayablePerformanceStat, EDBStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FDBScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FDBScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UDBSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	// DECLARE_EVENT_OneParam(UDBSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	...
	// void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UDBLocalPlayer* LocalPlayer);
	...
	// void ResetKeybindingToDefault(const FName MappingName, UDBLocalPlayer* LocalPlayer);
	...
	// void ResetKeybindingsToDefault(UDBLocalPlayer* LocalPlayer);
