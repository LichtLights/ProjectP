...
#include "DBSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EDBAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class EDBGamepadSensitivity : uint8
{
...
/**
 *	UDBSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UDBSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	
	DECLARE_EVENT_OneParam(UDBSettingsShared, FOnSettingChangedEvent, UDBSettingsShared* Settings);
...
public:
	...
	static UDBSettingsShared* CreateTemporarySettings(const UDBLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UDBSettingsShared* LoadOrCreateSettings(const UDBLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UDBSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UDBLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EDBAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EDBAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EDBAllowBackgroundAudioSetting AllowAudioInBackground = EDBAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EDBGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EDBGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EDBGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EDBGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EDBGamepadSensitivity GamepadLookSensitivityPreset = EDBGamepadSensitivity::Normal;
	UPROPERTY()
	EDBGamepadSensitivity GamepadTargetingSensitivityPreset = EDBGamepadSensitivity::Normal;
