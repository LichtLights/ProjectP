...
#include "System/Gameinstances/DBGameInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DBGameInstance)

// namespace DB
// {
// 	static bool bTestEncryption = false;
// 	static FAutoConsoleVariableRef CVarDBTestEncryption(
// 		TEXT("DB.TestEncryption"),
// 		bTestEncryption,
...
// 	static FAutoConsoleVariableRef CVarDBUseDTLSEncryption(
// 		TEXT("DB.UseDTLSEncryption"),
// 		bUseDTLSEncryption,
// 		TEXT("Set to true if using DB.TestEncryption and the DTLS packet handler."),
// 		ECVF_Default);
//
...
// 	static FAutoConsoleVariableRef CVarDBTestDTLSFingerprint(
// 		TEXT("DB.TestDTLSFingerprint"),
...
ADBPlayerController* UDBGameInstance::GetPrimaryPlayerController() const
{
	return Cast<ADBPlayerController>(Super::GetPrimaryPlayerController(false));
}

// bool UDBGameInstance::CanJoinRequestedSession() const
// {
...
// void UDBGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error,
...
// 	{
// 		UDBLocalPlayer* LocalPlayer = Cast<UDBLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
// 	
...
// void UDBGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken,
...
// 	{
// #if UE_WITH_DTLS
// 		if (DB::bUseDTLSEncryption)
// 		{
// 			TSharedPtr<FDTLSCertificate> Cert;
//
// 			if (DB::bTestDTLSFingerprint)
// 			{
...
// 				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("DBTest.pem");
//
...
// }
//
// void UDBGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
// {
// 	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
// 	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
// 	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UDBGameInstance::Init()
{
	...
	{
		ComponentManager->RegisterInitState(DBGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(DBGameplayTags::InitState_DataAvailable, false, DBGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(DBGameplayTags::InitState_DataInitialized, false, DBGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(DBGameplayTags::InitState_GameplayReady, false, DBGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UDBGameInstance::OnPreClientTravelToSession);
	}	
}
...
void UDBGameInstance::OnPreClientTravelToSession(FString& URL)
{
// 	// Add debug encryption token if desired.
// 	if (DB::bTestEncryption)
// 	{
// #if UE_WITH_DTLS
// 		if (DB::bUseDTLSEncryption)
// 		{
