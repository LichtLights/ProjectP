...
#include "System/GameFeatures/GameFeatureAction_AddInputBinding.h"
...
EDataValidationResult UGameFeatureAction_AddInputBinding::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const TSoftObjectPtr<const UDBInputConfig>& Entry : InputConfigs)
	{
		if (Entry.IsNull())
		{
	...
	FGameFeatureStateChangeContext ChangeContext)
{
	...
	else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == UDBHeroComponent::NAME_BindInputsNow))
	{
...
void UGameFeatureAction_AddInputBinding::AddInputMappingForPlayer(APawn* Pawn, FPerContextData& ActiveData)
{
	...
	if (ULocalPlayer* LocalPlayer = PlayerController ? PlayerController->GetLocalPlayer() : nullptr)
	{
		...
		{
			UDBHeroComponent* HeroComponent = Pawn->FindComponentByClass<UDBHeroComponent>();
			if (HeroComponent && HeroComponent->IsReadyToBindInputs())
			{
				for (const TSoftObjectPtr<const UDBInputConfig>& Entry : InputConfigs)
				{
					if (const UDBInputConfig* BindSet = Entry.Get())
					{
...
void UGameFeatureAction_AddInputBinding::RemoveInputMapping(APawn* Pawn, FPerContextData& ActiveData)
{
	...
	if (ULocalPlayer* LocalPlayer = PlayerController ? PlayerController->GetLocalPlayer() : nullptr)
	{
		if (UEnhancedInputLocalPlayerSubsystem* InputSystem = LocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
			if (UDBHeroComponent* HeroComponent = Pawn->FindComponentByClass<UDBHeroComponent>())
			{
				for (const TSoftObjectPtr<const UDBInputConfig>& Entry : InputConfigs)
				{
					if (const UDBInputConfig* InputConfig = Entry.Get())
					{
