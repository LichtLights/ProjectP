...
#include "System/GameFeatures/GameFeatureAction_AddAbilities.h"
...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(class FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const FDBAbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const FDBAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		for (const TSoftObjectPtr<const UDBAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
	...
	FGameFeatureStateChangeContext ChangeContext)
{
	...
	if (AbilitiesList.IsValidIndex(EntryIndex) && ActiveData)
	{
		...
		else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == ADBPlayerState::NAME_DBAbilityReady))
		{
	...
	FPerContextData& ActiveData)
{
	...
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry, ActiveData))
	{
		...
		for (const FDBAbilityGrant& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const FDBAttributeSetGrant& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
		...
		UDBAbilitySystemComponent* DBASC = CastChecked<UDBAbilitySystemComponent>(AbilitySystemComponent);
		for (const TSoftObjectPtr<const UDBAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
		{
			if (const UDBAbilitySet* Set = SetPtr.Get())
			{
				Set->GiveToAbilitySystem(DBASC, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
			}
		}
...
void UGameFeatureAction_AddAbilities::RemoveActorAbilities(AActor* Actor, FPerContextData& ActiveData)
{
	if (FActorExtensions* ActorExtensions = ActiveData.ActiveExtensions.Find(Actor))
	{
		if (UAbilitySystemComponent* AbilitySystemComponent = Actor->FindComponentByClass<UAbilitySystemComponent>())
		{
			...
			UDBAbilitySystemComponent* DBASC = CastChecked<UDBAbilitySystemComponent>(AbilitySystemComponent);
			for (FDBAbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
			{
				SetHandle.TakeFromAbilitySystem(DBASC);
			}
		}
