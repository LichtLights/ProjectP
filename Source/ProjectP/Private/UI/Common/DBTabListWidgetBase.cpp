...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DBTabListWidgetBase)

void UDBTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UDBTabListWidgetBase::NativeConstruct()
{
...
void UDBTabListWidgetBase::NativeDestruct()
{
	for (FDBTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UDBTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FDBTabDescriptor& OutTabInfo)
{
	const FDBTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FDBTabDescriptor& TabInfo) -> bool
	{
...
void UDBTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FDBTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UDBTabListWidgetBase::RegisterDynamicTab(const FDBTabDescriptor& TabDescriptor)
{
...
void UDBTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FDBTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UDBTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UDBTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FDBTabDescriptor* TabInfoPtr = nullptr;
	
	FDBTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UDBTabButtonInterface::StaticClass()))
	{
		...
		{
			IDBTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UDBTabListWidgetBase::IsFirstTabActive() const
{
...
bool UDBTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UDBTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UDBTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UDBTabListWidgetBase::SetupTabs()
{
	for (FDBTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
