...
#include "AbilitySystem/Executions/DBDamageExecution.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DBDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UDBCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
// #endif
	...
	FDBGameplayEffectContext* TypedContext = FDBGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	// TODO: Add faction etc. to kind of AttributeSet for calculate below.
	float DamageInteractionAllowedMultiplier = 0.0f;
	// if (HitActor)
	// {
	// 	UDBTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UDBTeamSubsystem>();
	...
	if (const IDBAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UDBStatusAttributeSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
